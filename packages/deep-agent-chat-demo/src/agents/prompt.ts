...
const EXECUTE_PREAMBLE = [
  ...
  'The Weather Forecast section must state that weather data is synthetic/demo randomized data.',
].join(' ');
...
export function buildPrompt(request: ChatRequest): string {
  ...
  if (request.phase === 'execute') {
    ...
    return [
      'You are running inside the isolated workspace for the deep-agent-chat-demo app.',
      'You must follow AGENTS.md and the .agents directory settings, rules, agents, and skills from this workspace.',
      EXECUTE_PREAMBLE,
  ...
  return [
    ...
    'Keep the orchestration JSON compact (minimal stage nodes) so the full fenced block fits in one response.',
    transcript ? `Recent transcript:\n${transcript}` : 'No recent transcript.',
    `Latest user message:\n${request.message}`,
  ].join('\n\n');
}
