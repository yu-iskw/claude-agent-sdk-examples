  ...
  const statusMessage = loading
    ? [
        loadingPhase === 'execute'
          ...
          : 'Orchestrating in DeepAgents plan profile, no tools running.',
        latestActivityLabel,
      ]
        .filter(Boolean)
        .join(' ')
      ...
      <div className="shell">
        ...
        >
          <header className="sidebar-header">
            <p className="eyebrow">DeepAgents Gemini Demo</p>
            <h1>Workspace-aware trip planner</h1>
            <p className="lede">
              A ChatGPT-style demo that constrains DeepAgents to an isolated app workspace with
              AGENTS.md settings, agents, skills, rules, and Context7 documentation access after
              approval.
            </p>
          </header>
          ...
          <div className="panel plan-panel plan-status-panel">
            ...
            <p className="plan-hint">
              Each send runs the DeepAgents <strong>plan profile</strong> first (no tools). The
              latest orchestration and orchestration graph appear <strong>in the chat</strong>.
              Approve, discard, and acknowledge the orchestration using the controls embedded in the
              active orchestration message card (checkbox + Approve &amp; run).
            </p>
          ...
          <div className="panel activity-panel">
            ...
            <p className="activity-hint muted">
              Streamed from DeepAgents runtime (tasks, tools, session init). This is{' '}
            ...
            {lastTrace ? (
              <ul>
                ...
                {lastTrace.availableAgents?.length ? (
                  <li>
                    <strong>Available agents (session init):</strong>{' '}
                    {lastTrace.availableAgents.join(', ')}
                  </li>
                ) : null}
                {lastTrace.availableSkills?.length ? (
                  <li>
                    <strong>Available skills (session init):</strong>{' '}
            ...
            {messages.map((message) => (
              <article className={`message ${message.role}`} key={message.id}>
                <span className="message-label">
                  {message.role === 'assistant' ? 'DeepAgent' : 'You'}
                </span>
                <PlanMessageBody
            ...
            {loading ? (
              <article aria-hidden="true" className="message assistant loading">
                {loadingPhase === 'execute'
                  ...
                  : 'Orchestration (DeepAgents plan profile, no tools)…'}
              </article>
            ) : null}
          </div>
