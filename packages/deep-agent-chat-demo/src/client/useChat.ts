...
const starterMessages: ChatMessage[] = [
  {
    id: 'welcome',
    role: 'assistant',
    text: 'Hi! I am a DeepAgents Gemini demo assistant. I run inside the app workspace so I can use AGENTS.md, .agents settings, markdown agents, skills, rules, and MCP tools after approval.\n\nSend a trip request to get an **orchestration** first (DeepAgents plan profile, no tools). The **orchestration prose and orchestration graph** appear **in this chat** with a **TODO checklist** (manual checkmarks plus optional activity-based estimates from live runtime events). For the latest orchestration, approval controls (checkbox + Approve & run) are embedded inside the orchestration message card. The workspace sidebar shows **live DeepAgents activity**, **orchestration status**, parse warnings, and **runtime trace**. Approval is not done by typing in chat.',
  },
];
